...
RC PredicatePushdownRewriter::rewrite(std::unique_ptr<LogicalOperator> &oper, bool &change_made)
{
    ...
    if(left_join_child_type == LogicalOperatorType::JOIN && right_join_child_type == LogicalOperatorType::TABLE_GET){
      ...
      for(auto it=pushdown_exprs.begin();it!=pushdown_exprs.end();){
        ...
        if(comparison_exper->left()->type()==ExprType::FIELD){
          auto left_exper = static_cast<FieldExpr*>(comparison_exper->left().get());
          related_to_right_table=related_to_right_table||(strcmp(left_exper->table_name(),right_table_name)==0);
        }
        if(comparison_exper->right()->type()==ExprType::FIELD){
          auto right_exper = static_cast<FieldExpr*>(comparison_exper->right().get());
          related_to_right_table=related_to_right_table||(strcmp(right_exper->table_name(),right_table_name)==0);
        }
      ...
      std::unique_ptr<ConjunctionExpr>conjunction_exper(new ConjunctionExpr(ConjunctionExpr::Type::AND,pushdown_exprs));
      std::unique_ptr<PredicateLogicalOperator>left_new_oper = std::unique_ptr<PredicateLogicalOperator>(new PredicateLogicalOperator(std::move(conjunction_exper)));
  ...
  if(predicate_expr==nullptr){
    LOG_TRACE("all expressions of predicate operator wwere pushdown to table get operator,then make a fake one");
